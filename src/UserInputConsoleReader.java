import Exeptions.ParseException;
...
public class UserInputConsoleReader implements UserInputReader {
    ...
    public UserData readUserData() throws UserDataException {
        while (true) {
            System.out.println("Введите данные в следующем порядке: Фамилия Имя Отчество Дата рождения (dd.mm.yyyy) Номер телефона Пол (f или m)");
            ...
            try {
                ...
                if (data.length != 6) {
                    throw new UserDataException("Введены не все данные");
                }
